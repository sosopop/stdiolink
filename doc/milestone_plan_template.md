...
- 单元测试覆盖采用“核心决策路径 100% + 非核心路径按风险分级覆盖”的策略；未覆盖路径需给出原因与补测计划，不可达路径需给出证明。
- 每个里程碑除单元测试外，必须提供对应的冒烟测试脚本，并可通过统一入口执行。
...
- 关键验收测试不得以 `DISABLED_`、注释或条件编译方式绕过执行。
- 关键验收路径原则上不得以 `skip` 替代通过判定；确需 `skip` 时必须记录环境约束、替代验证结果与恢复计划，且该路径不得计为“已通过”。
- 失败路径测试优先使用本地可控替身（mock/stub/fake），避免依赖公网或不可控外部环境。
...
覆盖要求（硬性）: 核心决策路径 `100%` 有用例；非核心路径按风险分级覆盖（高风险优先覆盖），未覆盖路径需写明原因与补测计划；不可达路径需写明原因与证明方式。

#### 执行约束（硬性）

- 被验收标准直接引用的用例 ID 必须实际执行，不得禁用。
- 被验收标准直接引用的用例 ID 原则上不得 `skip`；确需 `skip` 时必须提供环境约束说明、替代验证结果与恢复计划，且该用例不得计为“通过”。
- 每条失败路径测试需声明“本地可复现触发条件”。
...
- 脚本文件: `<mXX_*.py>`
- 统一入口: `python src/smoke_tests/run_smoke.py --plan <PLAN_NAME>`
- CTest 接入: `<ctest 测试名>`（示例：`smoke_mXX_*`）
- 覆盖范围: <最小可运行主链路 + 关键失败路径>
- 用例清单:
  - `<S01>`: <场景> -> <断言>
  - `<S02>`: <场景> -> <断言>
- 失败输出规范: <stdout/stderr/退出码/超时信息>
- 环境约束与跳过策略: <依赖不存在时 skip 条件，禁止误报通过>
- 产物定位契约: <可执行文件输出名、路径规则（runtime/raw 布局）>
- 跨平台运行契约: <PATH 注入策略、编码策略（Windows/Linux/macOS）>
- 可执行文件不存在时必须判定 `FAIL` 并输出候选路径，禁止静默通过

### 6.3 集成/端到端测试（涉及跨模块交互或跨进程通信时必填）
- <跨模块联动>
- <协议/API/进程生命周期联调>
- <关键用户流程 E2E（前端或发布链路）>
...
- 当依赖关系满足以下任一条件时，必须包含”依赖关系”章节（放 `8` 后附录）：跨团队协作、跨仓库依赖、公共接口/协议强耦合。其余同仓轻依赖可在“前置条件”中简写说明。
...
- [ ] 核心决策路径均被单元测试覆盖，非核心路径按风险分级覆盖；每条已覆盖路径可追溯到具体测试用例 ID。
- [ ] 对声明”不可达”的路径提供设计级证明（状态机约束/参数前置条件/防御式分支）。
- [ ] 已定义冒烟测试脚本路径、执行入口、CTest 名称与覆盖场景。
- [ ] 冒烟脚本覆盖至少一个主成功链路和一个关键失败链路（或明确说明无失败链路）。
- [ ] 验收标准引用的测试用例不存在 `DISABLED_`/注释禁用/条件禁用。
- [ ] 验收标准引用的测试用例若出现 `SKIP`，已附环境约束、替代验证结果与恢复计划，且未被计为“通过”。
- [ ] 失败路径测试触发条件为本地可控场景（非公网依赖）。
- [ ] 冒烟脚本中可执行文件输出名与路径规则（runtime/raw）已明确并可校验。
...
- 冒烟测试脚本必须使用统一入口注册（`src/smoke_tests/run_smoke.py`），禁止只给散落脚本路径。
- 冒烟测试脚本必须可在本地命令行直接运行，并在 CTest 中可被发现与执行。
- 冒烟测试断言必须包含退出码、关键输出字段或外部可观察状态，禁止仅以“脚本未崩溃”判定通过。
- 冒烟测试需写明 skip 条件及原因（如硬件/端口/外部服务依赖），skip 不能替代失败断言。
- 关键验收路径测试原则上不得使用 `skip` 作为通过策略；确需 `skip` 时必须给出替代验证与恢复计划，并在报告中标记“未通过”。
- 测试中的早退分支（`return`/`ASSERT_*`/`SKIP`）必须保证资源清理（进程、端口、临时文件、线程）。
- 子进程/流式输出测试需给出尾包读取策略，避免进程快速退出导致漏读。
