...
本目录覆盖本地构建、测试矩阵、发布打包、Docker 运行镜像和调试入口。
...
- `test-artifact-and-runtime-layout.md`：解释 `build/`、`runtime_*` 和发布目录的边界。
- `ctest-vs-direct-run.md`：解释 `ctest` 注册命令和手动运行测试程序的差异。
- `stream-transport-test-pattern.md`：半包、粘包、分段到达这类流式接收问题的测试入口。
