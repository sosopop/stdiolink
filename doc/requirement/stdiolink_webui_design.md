...
│  │ [级别: 全部▼] [搜索...] 🔄 [自动滚动☑] [清空] [导出]│ │
│  ├────────────────────────────────────────────────────┤ │
│  │ 11:23:45.123 INFO  Service started on port 6200   │ │
...
```bash
VITE_API_BASE_URL=http://localhost:6200
VITE_WS_BASE_URL=ws://localhost:6200
```
...
- 可选：访问 `http://localhost:6200/` 自动跳转到 `/webui/`

**方案2：独立部署（当前代码基线推荐）**
```
[Nginx] ──┬──> /api/* ──> stdiolink_server:6200
          ├──> /api/driverlab/* ──> stdiolink_server:6200
          └──> /* ──> WebUI (静态文件)
```
...
# 启动服务
CMD ["/app/stdiolink_server", "--port=6200", "--host=0.0.0.0"]
```
