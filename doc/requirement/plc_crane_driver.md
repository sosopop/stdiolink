...
- **Modbus 操作**：读离散量输入 (FC 0x02)，地址 9–14（一次读取 6 个输入位，取 9/10/13/14）
- **参数**：
...
| 字段 | 离散量地址 | 原始位值 | 解释 |
|------|------------|----------|------|
| cylinder_up | 9 | `0/1` | `0=到位`，`1=未到位`（低电平有效） |
| cylinder_down | 10 | `0/1` | `0=未到位`，`1=到位` |
| valve_open | 13 | `0/1` | `0=未到位`，`1=到位` |
| valve_closed | 14 | `0/1` | `0=未到位`，`1=到位` |
