...
为确保 Services 模块符合 **Style 06 (Premium Glass)** 的设计标准：

1.  **列表视图 (List View)**：
    *   **玻璃态容器**：表格容器应应用 `glass-panel` 样式。表头背景色应设为极低透明度（如 `rgba(255,255,255,0.02)`），让背景的径向渐变能微微透出。
    *   **行分割**：移除显式的行边框，改用斑马纹（Alternating Rows）或仅在悬停时显示 `surface-hover` 背景色，保持界面的通透感。

2.  **文件管理器 (File Manager)**：
    *   **光晕选中态**：文件树的选中状态不要使用实色背景。建议使用 `background: var(--brand-glow)` (低透明度品牌色) 配合左侧 `3px` 宽的品牌色亮条，营造发光选中的感觉。
    *   **图标质感**：文件夹图标建议使用单色线条风格，颜色采用 `var(--text-secondary)`，避免高饱和度彩色图标破坏整体的高级灰调。

3.  **编辑器集成 (Editor Immersion)**：
    *   **背景融合**：Monaco Editor 的默认背景色通常是纯黑/纯灰。请将其背景设为 `transparent`，或者强制设置为 `var(--surface-card)`，使其与外层的玻璃态容器完美融合，消除“补丁感”。

4.  **空状态 (Empty States)**：
    *   **氛围感**：扫描为空时，展示一个带有微弱渐变填充的 SVG 插画，并配以“开始你的第一个服务”的引导按钮（使用 Primary Gradient）。
