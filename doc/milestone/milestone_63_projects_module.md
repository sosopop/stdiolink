...
Projects 模块涉及复杂的配置与管理，设计重点在于降低认知负荷：

1.  **创建向导 (Creation Wizard)**：
    *   **进度感知**：Steps 组件应清晰指示当前位置。完成的步骤使用实心圆点，未完成的使用空心圆点。
    *   **聚焦内容**：向导模态框应移除不必要的干扰元素，让用户专注于当前的配置任务。
    *   **即时校验**：错误提示应以内联文本形式（`text-error`）出现在字段下方，避免使用弹窗打断用户流。

2.  **智能表单 (Schema Form)**：
    *   **分组层级**：使用 `Collapse` 面板对配置进行分组时，边框应极简 (`1px solid rgba(255,255,255,0.06)`)，展开动画需流畅（300ms cubic-bezier）。
    *   **间距控制**：表单字段间的垂直间距保持 `24px`，Label 与 Input 间距 `8px`。
    *   **视觉降噪**：Label 颜色应略淡于 Input 内容 (`text-gray-400` vs `text-white`)，以突出用户输入的数据。

3.  **日志查看器 (Log Viewer)**：
    *   **阅读体验**：必须使用等宽字体 `JetBrains Mono`。
    *   **行高设定**：设置行高为 `1.5`，避免密集文本造成的视觉疲劳。
    *   **对比度**：日志时间戳建议使用暗灰色，与日志内容区分开。ERROR 级别的日志背景可以使用极淡的红色 (`rgba(239, 68, 68, 0.1)`) 整行高亮，而不仅仅是文字变红。
