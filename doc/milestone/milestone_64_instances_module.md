...
Instances 模块是系统的“心跳”，界面应传达出实时、动态且可控的感觉：

1.  **状态呼吸灯 (Breathing Status)**：
    *   **克制动画**：运行中状态的绿色光晕动画周期应较长（如 3s），透明度变化范围控制在 `0.4` 到 `1.0` 之间，避免像警报灯一样闪烁，造成焦虑感。
    *   **色彩语义**：严格遵循 Semantic 颜色规范，Success (Emerald), Warning (Amber), Error (Red)。

2.  **资源图表 (Resource Charts)**：
    *   **极简图表**：移除图表背景的网格线，或将其透明度降至极低 (`0.05`)。
    *   **渐变填充**：面积图 (Area Chart) 的填充色应使用从 Primary 色到完全透明的垂直线性渐变，营造现代科技感。
    *   **动态平滑**：数据更新时应开启 `isAnimationActive`，确保曲线平滑推进，避免跳变。

3.  **进程树 (Process Tree)**：
    *   **连接线**：树形结构的连接线应细致 (`1px`) 且低对比度，起到引导视线的作用即可，不要喧宾夺主。
    *   **告警视觉**：CPU 占用过高时的红色文字应谨慎使用。如果整棵树都变红会非常刺眼，建议仅高亮数值部分，或在该行左侧添加细微的红色竖条标记。
