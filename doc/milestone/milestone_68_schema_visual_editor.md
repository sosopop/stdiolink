...
Schema 编辑器是高复杂度的配置工具，设计目标是清晰与直观：

1.  **层级可视化 (Visual Hierarchy)**：
    *   **嵌套缩进**：对象（Object）字段的子属性应有明显的左侧缩进 (`24px` 或更多)，并使用垂直连接线辅助识别层级关系。
    *   **背景区分**：深层嵌套的容器背景色应逐级变深（或变亮，取决于具体设计），形成“卡片中的卡片”的视觉包裹感，防止用户迷失层级。

2.  **交互细节 (Interaction Details)**：
    *   **添加字段**：在对象容器底部提供“添加子字段”的虚线框按钮，hover 时高亮，明确告知用户添加的位置。
    *   **拖拽排序**：如果支持排序，拖拽把手（Drag Handle）应始终可见，且拖拽过程中被拖动元素应有阴影浮起效果。

3.  **模式同步反馈 (Sync Feedback)**：
    *   **无缝切换**：在“可视化”和“JSON”模式切换时，不应有明显的页面闪烁。
    *   **错误标记**：当 JSON 模式存在语法错误导致无法同步回可视化模式时，Tab 标签上应显示红点或警告图标，提示用户当前处于“仅文本编辑”状态，可视化视图已暂停更新。
