...
Drivers 模块主要面向开发者，设计应侧重于文档的易读性和信息的结构化展示，同时保持 **Style 06** 的通透感：

1.  **文档容器 (Doc Container)**：
    *   **玻璃态阅读区**：Markdown 文档应渲染在一个独立的 `glass-panel` 容器中，内边距至少为 `32px`。这种“悬浮纸张”的效果能让文档与复杂的背景分离，提升阅读专注度。
    *   **排版节奏**：标题 (`h1`-`h3`) 下方增加 `margin-bottom`，利用负空间引导视线。

2.  **代码块样式 (Code Blocks)**：
    *   **高对比度**：由于背景是半透明的玻璃态，代码块背景建议使用较深的半透明色 (`rgba(0,0,0,0.3)`)，以确保代码高亮的颜色能准确呈现，不受背景杂色干扰。
    *   **圆角**：代码块圆角应与全局 `radius-sm` (8px) 保持一致。

3.  **操作按钮 (Ghost Actions)**：
    *   **幽灵按钮**：导出按钮建议使用 Ghost 风格（无填充、仅边框）。悬停时，边框颜色渐变为 Brand Gradient，并产生微弱的辉光，避免在阅读型页面中喧宾夺主。
