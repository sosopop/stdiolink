...
Projects 模块涉及复杂的配置与管理，设计重点在于降低认知负荷的同时保持视觉的愉悦感：

1.  **玻璃态向导 (Glass Wizard)**：
    *   **模态框质感**：创建向导的 Modal 弹窗应使用 `glass-panel` 样式。遮罩层 (`overlay`) 建议使用较重的模糊 `backdrop-filter: blur(16px)`，将背景噪音完全过滤，让用户聚焦于当前任务。
    *   **步骤条 (Steps)**：步骤组件的激活状态应使用 `var(--brand-gradient)`，连接线使用低透明度的品牌色。

2.  **智能表单 (Schema Form)**：
    *   **光晕聚焦**：所有输入框 (Input/Select) 在聚焦时，应产生扩散的 `var(--brand-glow)` 阴影，而不是锐利的实线边框。这能带来更柔和的“触控”感。
    *   **分组层级**：`Collapse` 面板的背景建议使用极低透明度的白色 (`rgba(255,255,255,0.02)`)，通过微弱的亮度差来区分层级，而非依赖边框。

3.  **日志查看器 (Log Viewer)**：
    *   **沉浸阅读**：日志区域建议使用比 `glass-panel` 更深一点的半透明背景 (`rgba(0,0,0,0.3)`)，以提高文字对比度。
    *   **字体规范**：强制使用 `JetBrains Mono`，利用其优秀的连字特性提升代码阅读体验。
