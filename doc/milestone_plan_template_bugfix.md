...
> 适用路线：问题修复/缺陷回归；新需求开发请使用 `doc/milestone_plan_template_new_feature.md`
...
1. 问题修复/缺陷回归：用"问题修复 TDD 模板"，`4` 章强制使用 Red-Green-Refactor 三段式。
2. 仅核心库能力变更：若属于新需求，请改用 `doc/milestone_plan_template_new_feature.md`。
3. 以 JS 内置模块/绑定为主：若属于新需求，请改用 `doc/milestone_plan_template_new_feature.md`。
4. 以 HTTP/API/编排为主：若属于新需求，请改用 `doc/milestone_plan_template_new_feature.md`。
5. 以页面交互为主：若属于新需求，请改用 `doc/milestone_plan_template_new_feature.md`。
6. 跨栈里程碑：若属于新需求，请改用 `doc/milestone_plan_template_new_feature.md`，主模板不变并在 `1` 章先给"子系统交付矩阵"（前端/后端/脚本/测试）。
