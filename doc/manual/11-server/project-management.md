...
Project 是对某个 Service 的一次实例化配置，包含业务参数和调度策略。每个 Project 对应 `projects/` 目录下的一个子目录。
...
| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `id` | string | 是 | Project 稳定标识，需与目录名一致 |
| `name` | string | 是 | Project 显示名称 |
| `serviceId` | string | 是 | 关联的 Service ID |
| `enabled` | bool | 否 | 是否启用，默认 `true` |
| `schedule` | object | 是 | 调度策略配置 |

`param.json` 为业务配置参数对象，由 Service 的 Schema 校验。

## Project ID 规则

- Project ID 取自目录名，如 `projects/silo-a/` → ID 为 `silo-a`
- 必须匹配正则 `^[A-Za-z0-9_-]+$`
- 禁止包含 `/`、`\`、`..`、空白字符
- 文件路径固定为 `projects/{id}/config.json`，参数文件固定为 `projects/{id}/param.json`
...
```
1. 解析 `config.json`，提取 serviceId
2. 查找对应的 ServiceInfo（来自 ServiceScanner）
3. 若 Service 不存在 → 标记为 invalid
4. 读取 `param.json`（缺失时按空对象）
5. 使用 ServiceConfigValidator 验证参数对象
6. 解析 schedule 参数并校验约束
7. 全部通过 → valid，任一失败 → invalid + 记录 error
```
...
直接在 `projects/{id}/` 目录下创建、编辑、删除 `config.json` / `param.json`。修改后需通过 API 触发重载：
