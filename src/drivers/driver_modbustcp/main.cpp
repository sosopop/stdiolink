...
public:
    ...
    ModbusClient* getClient(const QString& host, quint16 port, bool keepalive, int timeout) {
        ...
        m_tempClient = std::make_shared<ModbusClient>(timeout);
...
private:
    ...
    std::shared_ptr<ModbusClient> m_tempClient;
};
...
void ModbusTcpHandler::buildMeta()
{
    m_meta = DriverMetaBuilder()
        ...
        .info("modbus.tcp", "ModbusTCP Master", "1.0.0",
              "Modbus TCP 主机驱动，支持读写线圈、离散输入、保持寄存器和输入寄存器")
        .vendor("stdiolink")
