...
static FieldBuilder connectionParam(const QString& name) {
    if (name == "host") {
        return FieldBuilder("host", FieldType::String)
            .required()
            .description("Modbus RTU Over TCP 网关地址，如 192.168.1.100")
    ...
    if (name == "port") {
        return FieldBuilder("port", FieldType::Int)
            .defaultValue(502)
            .range(1, 65535)
            .description("Modbus RTU Over TCP 端口，默认 502");
    }
    if (name == "unit_id") {
...
void ModbusRtuHandler::buildMeta()
{
    ...
    m_meta = DriverMetaBuilder()
        ...
        .info("modbus.rtu", "ModbusRTU Over TCP Master", "1.0.0",
              "Modbus RTU Over TCP 主站驱动，使用 RTU 帧格式（带 CRC16）通过 TCP 与远端网关通信")
        .vendor("stdiolink")
