...
if(EXISTS "${CURRENT_PACKAGES_DIR}/usr/local")
    file(GLOB ICU_USR_LOCAL_CONTENT LIST_DIRECTORIES true "${CURRENT_PACKAGES_DIR}/usr/local/*")
    if(ICU_USR_LOCAL_CONTENT)
        file(COPY ${ICU_USR_LOCAL_CONTENT} DESTINATION "${CURRENT_PACKAGES_DIR}")
    endif()
...
if(VCPKG_TARGET_IS_LINUX OR VCPKG_TARGET_IS_OSX AND "tools" IN_LIST FEATURES)
    set(ICU_TOOLS_SEARCH_DIR "${CURRENT_PACKAGES_DIR}/tools/icu/sbin")
    if(NOT IS_DIRECTORY "${ICU_TOOLS_SEARCH_DIR}")
        set(ICU_TOOLS_SEARCH_DIR "${CURRENT_PACKAGES_DIR}/sbin")
    endif()
    vcpkg_copy_tools(
        TOOL_NAMES icupkg gennorm2 gencmn genccode gensprep
        SEARCH_DIR "${ICU_TOOLS_SEARCH_DIR}"
...
set(ICU_CONFIG_PATH "${CURRENT_PACKAGES_DIR}/tools/icu/bin/icu-config")
if(NOT EXISTS "${ICU_CONFIG_PATH}")
    set(ICU_CONFIG_PATH "${CURRENT_PACKAGES_DIR}/bin/icu-config")
endif()
vcpkg_replace_string("${ICU_CONFIG_PATH}" "${CURRENT_INSTALLED_DIR}" "`dirname $0`/../../../" IGNORE_UNCHANGED)
vcpkg_replace_string("${ICU_CONFIG_PATH}" "${CURRENT_HOST_INSTALLED_DIR}" "`dirname $0`/../../../../${_HOST_TRIPLET}/" IGNORE_UNCHANGED)
